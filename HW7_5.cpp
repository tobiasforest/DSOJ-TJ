...
public:
    ALGraph(const GraphType t, const int vn, const int an)
        : type(t), vexNum(vn), arcNum(an), vertices(new VNode[vexNum]), visited(new int[vexNum]), indegree(new int[vexNum])
    {
        for (int i = 0; i < vexNum; )
        {
            vertices[i].data = (VertexType)++i;
        }
    ...
    int topologicalSort()
    {
        ...
        while (!s.empty())
        {
            ...
            {
                int k = p->adjvex; // index of adjacent vertix
                if (!--indegree[k])
                {
                    s.push(k);
                }
